...
字段的含义第一行已经标示出来,每一行代表一个网卡,adns_tsar主要采集的是出口和入口的bytes／packets  
注意adns_tsar只对以eth和em开头的网卡数据进行了采集,像lo这种网卡直接就忽略掉了,流量的单位是byte
...
然后就可以计算出adns_tsar需要的信息,分区的字节数＝块数＊块大小＝f_blocks * f_bsize
...
注意,需要将采集的进程名称配置在/etc/adns_tsar/adns_tsar.conf总的mod_proc on procname,这样就会找到procname的pid,并进行数据采集
...
####采集方法
通过nginx的采集模块配置,访问特定地址,具体参见:https://github.com/taobao/adns_tsar-mod_nginx  
...
如果nginx的端口不是80,则需要在配置文件中指定端口,配置文件是/etc/adns_tsar/adns_tsar.conf,修改mod_nginx on为mod_nginx on 8080  
...
在/etc/adns_tsar/adns_tsar.conf中指定配置文件的路径:mod_nginx_domain on /tmp/my.conf  
...
###apache
参见:https://github.com/kongjian/adns_tsar-apache
###tcprt
私有应用,略
###swift
私有应用,略
