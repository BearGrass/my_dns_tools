...
create_script()
{
    ...
    FILE *fp = fopen("/tmp/rndc_adns_tsar.pl", "w");
    if (!fp) {
        return;
    }
    fputs(script, fp);
...
exec_script()
{
    ...
    if (system("perl /tmp/rndc_adns_tsar.pl > /tmp/rndc_adns_tsar.txt") < 0) {
        exit(-1);
    }
    if (system("echo -n 'badvs,' >> /tmp/rndc_adns_tsar.txt; MYSQL_BIN=`/bin/rpm -ql mysql|/bin/egrep -e '/bin/mysql$'` && ${MYSQL_BIN} -ss -uroot -Ddns_config -e 'SELECT COUNT(name) FROM vs WHERE in_use=1 AND available=0' >> /tmp/rndc_adns_tsar.txt") < 0) {
        exit(-1);
    }
}
...
{
    FILE *fp = fopen("/tmp/rndc_adns_tsar.txt", "r");
    if (!fp) {
        return;
    }
    ...
    fclose(fp);
    if (system("rm /tmp/rndc_adns_tsar.txt") < 0){
        exit(-1);
    }
