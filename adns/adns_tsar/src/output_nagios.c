...
#define PRE_RECORD_FILE     "/tmp/.adns_tsar.tmp"
...
output_nagios()
{
                ...
                while (token) {
                    ...
                    for (k = 0; k < mod->n_col; k++) {
                        ...
                        for (l = 0; l < conf.mod_num; l++){
                            /* cmp adns_tsar item with naigos item*/
    ...
    sprintf(nagios_cmd, "echo \"%s;adns_tsar;%d;%s|%s\"|%s -H %s -p %d -to 10 -d \";\" -c %s", host_name, result, output_err, output, conf.send_nsca_cmd, conf.server_addr, conf.server_port, conf.send_nsca_conf);
