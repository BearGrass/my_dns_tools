 ...
 * return
 * 0 ok
 * 1 need find last adns_tsar.data file
 * 2 find error, find time is later than the last line at adns_tsar.data.x, should stop find any more
 * 3 find error, adns_tsar haved stopped after find time, should stop find it
 * 4 find error, data not exist, adns_tsar just lost some time data which contains find time
 * 5 log format error
 * 6 other error
 */
int
find_offset_from_start(FILE *fp, int number)
{
    ...
    while (1) {
        ...
        if (0 != line[0] && offset > line_len) {
            ...
            if (off_end == file_len) {
                if (number > 0) {
                    ...
                    /* at the end of adns_tsar.data.%d have some data lost during data rotate. stat from previous log file";*/
                    return 2;

                } else {
                    /* researching adns_tsar.data to end and not find log data you need.";*/
                    return 3;
                }
            }
            if (off_start == 0) {
                conf.print_file_number = number;
                /* need to research adns_tsar.data.number+1; */
                return 1;
            }
        ...
        if (offset == (off_start + off_end) / 2) {
            if (off_start != 0) {
                /* adns_tsar has been down for a while, so the following time's stat we can provied only; */
                conf.print_file_number = number;
                return 4;
            }
            return 6;
...
/*
 * check time if corret for pirnt from adns_tsar.data
 */
int
...
init_running_print()
{
    ...
    /* find start offset will print from adns_tsar.data */
...
/*
 * print mode, print data from adns_tsar.data
 */
void
running_print()
{
...
running_check(int check_type)
{
    ...
    if (0 != gethostname(host_name, sizeof(host_name))) {
        do_debug(LOG_FATAL, "adns_tsar -check: gethostname err, errno=%d", errno);
    }
    i = 0;
    while (host_name[i]) {
    ...
    sprintf(check, "%s\tadns_tsar\t", host_name);
    sprintf(filename, "%s", conf.output_file_path);
    fp = fopen(filename, "r");
    if (!fp) {
        do_debug(LOG_FATAL, "unable to open the log file %s.\n", filename);
    ...
    if (nowtime - statbuf.st_ctime > 300) {
        do_debug(LOG_FATAL, "/var/log/adns_tsar.data is far away from now, last time is %s", ctime(&statbuf.st_ctime));
    }
    /* get file len */
    ...
    if (total_num == 0) {
        ...
        total_num = 0;
        /* go to the start of the last line at adns_tsar.data.1 */
        if (fseek(fp, -1, SEEK_END) != 0) {
            do_debug(LOG_FATAL, "fseek error:%s", strerror(errno));
        }
        while (1) {
...
#ifdef OLDTSAR
    /*adns_tsar -check output similar as:
      v014119.cm3   adns_tsar   apache/qps=5.35 apache/rt=165.89 apache/busy=2 apache/idle=148 cpu=3.58 mem=74.93% load1=0.22 load5=0.27 load15=0.20 xvda=0.15 ifin=131.82 ifout=108.86 TCPretr=0.12 df/=4.04% df/home=10.00% df/opt=71.22% df/tmp=2.07% df/usr=21.27% df/var=5.19%
     */
    /* ------------------------------RUN_CHECK------------------------------------------- */
