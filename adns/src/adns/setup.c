...
void adns_usage(const char *prgname)
{
    printf("Usage: %s [EAL options] -- -p PORTMASK [-h] [-f conf_file]\n"
            ...
            "  -f filename  Configuture file for adns\n"
...
int get_lcore_for_nic_rx(uint8_t port, uint8_t queue, uint32_t *lcore_out)
{
    ...
    for (lcore = 0; lcore < RTE_MAX_LCORE; lcore++) {
        ...
        if (app.lcore_params[lcore].type == e_LCORE_MISC) {
            if (queue != 0)
                continue;
            *lcore_out = lcore;
            return 0;
...
int get_lcore_for_nic_tx(uint8_t port, uint8_t queue, uint32_t *lcore_out)
{
    ...
    for (lcore = 0; lcore < RTE_MAX_LCORE; lcore++) {
        ...
        for (i = 0; i < lp->n_tx_ports; i++) {
            if ((lp->tx_ports[i] == port) &&
                    (lp->tx_queues[lp->tx_ports[i]] == queue)) {
                *lcore_out = lcore;
                return 0;
            }
        }
...
int get_io_lcore_start_id() {
    ...
    for (i = 0; i < RTE_MAX_LCORE; i++) {
        if (app.lcore_params[i].type == e_LCORE_IO) {
            return i;
        }
    }
    return -1;
...
int adns_parse_args(int argc, char **argv)
{
    ...
    while ((opt = getopt_long(argc, argvopt, "p:f:mh",
...
void app_print_params(void)
{
    ...
    for (lcore = 0; lcore < RTE_MAX_LCORE; lcore ++) {
        ...
        if (((app.lcore_params[lcore].type != e_LCORE_IO)
                    && (app.lcore_params[lcore].type) != e_LCORE_MISC)
                || (lp->n_rx_queues == 0)) {
            continue;
        }

        printf("I/O|MISC lcore %u (socket %u): ", lcore, rte_lcore_to_socket_id(lcore));

        printf("TX ports  ");
        for (i = 0; i < lp->n_tx_ports; i ++) {
            printf("(%u, %u)  ",
                    (unsigned) lp->tx_ports[i],
                    (unsigned) lp->tx_queues[lp->tx_ports[i]]);
        }
