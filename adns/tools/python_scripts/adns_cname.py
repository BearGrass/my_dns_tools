...
if __name__ == "__main__":      
  ...
  for domain in result:
    single_chain = []
    single_chain.append(domain)
    single_chain.append(result[domain])

    next = result[domain]
    while(result.has_key(next) and (single_chain.count(result[next]) == 0) ):
      single_chain.append(result[next])
      next = result[next]

    chain_list.append(single_chain)
    #print str(len(single_chain)) + " " + str(single_chain)
  ...
  for single_chain in chain_list:
    print str(len(single_chain)) + " " + str(single_chain)
  #for domain in result:
  #    print str(domain) + " " + str(result[domain])
