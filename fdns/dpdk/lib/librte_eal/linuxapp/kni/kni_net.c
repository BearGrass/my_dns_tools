 ...
 * Ioctl commands
 *
 * The ndo_change_carrier callback is not support on 2.6 kernel,
 * so use ioctl to control the carrier on and off.
 */
static int
kni_net_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
    ...
    if (cmd == SIOCDEVKNICARON)
        netif_carrier_on(dev);
    else if (cmd == SIOCDEVKNICAROFF)
