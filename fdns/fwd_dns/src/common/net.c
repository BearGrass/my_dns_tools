...
int conn_tx_off(struct connection *conn)
{
    ...
    return event_modify(conn->fd, conn->events);
}
...
int set_nonblocking(int fd)
{
    ...
    if (ret < 0) {
        ...
        if (ret < 0)
            printf("fcntl O_NONBLOCK failed: %m\n");
    }
...
rewrite:
    ...
    if (ret < 0) {
        if (errno == EINTR)
            goto rewrite;
        printf("failed to write to socket: %m\n");
        return 1;
    }
...
int send_req(int sockfd, struct drm_req *hdr, void *data, unsigned int *wlen)
{
    ...
    if (ret) {
        printf("failed to send request %x, %d: %m\n", hdr->opcode, *wlen);
        ret = -1;
    }
...
{
    int ret;
    struct drm_rsp *rsp = (struct drm_rsp *)hdr;
    ...
    if (ret) {
        printf("failed to read a response\n");
        return 1;
    }

    if (*rlen > rsp->data_len)
        *rlen = rsp->data_len;

    if (*rlen) {
        ret = do_read(sockfd, data, *rlen);
        if (ret) {
            printf("failed to read the response data\n");
            return 1;
        }
    }
...
int connect_to(const char *name, int port)
{
    ...
    if (ret) {
        printf("failed to get address info: %m\n");
        return -1;
    }
    ...
    for (res = res0; res; res = res->ai_next) {
        ...
        ret = setsockopt(fd, SOL_SOCKET, SO_LINGER, &linger_opt,
                         sizeof(linger_opt));
        if (ret) {
            printf("failed to set SO_LINGER: %m\n");
            close(fd);
            continue;
        }
        ...
        if (ret) {
            printf("failed to connect to %s:%d: %m\n", name, port);
            close(fd);
            continue;
        }
...
int create_listen_ports(int port, int (*callback) (int fd, void *), void *data)
{
    ...
    for (res = res0; res; res = res->ai_next) {
        ...
        if (ret)
            printf("failed to set SO_REUSEADDR: %m\n");
        ...
        if (ret) {
            printf("failed to bind server socket: %m\n");
            close(fd);
            continue;
        }
        ...
        if (ret) {
            printf("failed to listen on server socket: %m\n");
            close(fd);
            continue;
        }
    ...
    if (!success)
        printf("failed to create a listening port\n");
    ...
    for (ifa = ifaddr; ifa; ifa = ifa->ifa_next) {
        ...
        switch (ifa->ifa_addr->sa_family) {
            case AF_INET:
                ...
                memcpy(bytes + 12, &sin->sin_addr, 4);
                printf("found IPv4 address\n");
                goto out;
            case AF_INET6:
                sin6 = (struct sockaddr_in6 *)ifa->ifa_addr;
                memcpy(bytes, &sin6->sin6_addr, 16);
                printf("found IPv6 address\n");
                goto out;
        }
    }
...
int parse_mac_str(const char *arg, char *mac)
{
    int ret;
    const char *p1;
    const char *p2;
