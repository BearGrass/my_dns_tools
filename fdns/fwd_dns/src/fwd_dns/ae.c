...
static int aeApiPoll(aeEventLoop * eventLoop, const struct timeval *tvp)
{
    aeApiState *state = eventLoop->apidata;
...
int aeResizeSetSize(aeEventLoop * eventLoop, int setsize)
{
    ...
    eventLoop->events =
        realloc(eventLoop->events, sizeof(aeFileEvent) * setsize);
    eventLoop->fired =
        realloc(eventLoop->fired, sizeof(aeFiredEvent) * setsize);
    eventLoop->setsize = setsize;
...
int aeMain(aeEventLoop * eventLoop)
{
    ...
    while (!eventLoop->stop) {
        ret = aeProcessEvents(eventLoop, AE_ALL_EVENTS);
        eventLoop->stop = 1;
    }
