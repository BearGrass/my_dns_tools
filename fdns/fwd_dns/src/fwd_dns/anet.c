...
int anet_set_cork_on(int fd)
{
    return set_cork(fd, 1);
}

int anet_set_cork_off(int fd)
{
...
static int anetCreateSocket(char *err, int domain)
{
    ...
    if ((s = socket_linux(domain, SOCK_STREAM, 0)) == -1) {
...
static int anetTcpGenericConnect(char *err, char *addr, int port, int flags)
{
    ...
    for (p = servinfo; p != NULL; p = p->ai_next) {
        if ((s = socket_linux(p->ai_family, p->ai_socktype, p->ai_protocol)) == -1)
            continue;
...
int anetTcpAccept(char *err, int s, char *ip, size_t ip_len, int *port)
{
    ...
    if ((fd =
         anetGenericAccept(err, s, (struct sockaddr *)&sa, &salen)) == ANET_ERR)
        return ANET_ERR;
