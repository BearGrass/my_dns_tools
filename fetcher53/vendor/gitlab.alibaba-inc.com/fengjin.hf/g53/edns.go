...
func EdnsFromWire(buf *buffer.InputBuffer) (*EDNS, error) {
...
func (e *EDNS) FromRRset(rrset *RRset) error {
	...
	if len(rrset.Rdatas) > 0 {
		for _, rdata := range rrset.Rdatas {
			...
			buf := buffer.NewInputBuffer(opt.Data)
...
func (e *EDNS) Rend(r *MsgRender) {
	...
	if len(e.Options) == 0 {
		...
		r.WriteUint16At(pos, uint16(r.Len()-pos-2))
	}
}

func (e *EDNS) ToWire(buf *buffer.OutputBuffer) {
	flags := uint32(e.extendedRcode) << EXTRCODE_SHIFT
	flags |= (uint32(e.Version) << VERSION_SHIFT) & VERSION_MASK
	if e.DnssecAware {
		flags |= EXTFLAG_DO
