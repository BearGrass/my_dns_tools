...
func (ds *DS) Rend(r *MsgRender) error {
	if err := rendField(RDF_C_UINT16, ds.KeyTag, r); err != nil {
		return err
	}

	if err := rendField(RDF_C_UINT8, ds.Algorithm, r); err != nil {
		return err
	}

	if err := rendField(RDF_C_UINT8, ds.DigestType, r); err != nil {
		return err
	}

	return rendField(RDF_C_BINARY, encodeStringToHex(ds.Digest), r)
}

func (ds *DS) ToWire(buf *buffer.OutputBuffer) error {
	if err := fieldToWire(RDF_C_UINT16, ds.KeyTag, buf); err != nil {
		return err
	}
	if err := fieldToWire(RDF_C_UINT8, ds.Algorithm, buf); err != nil {
		return err
	}

	if err := fieldToWire(RDF_C_UINT8, ds.DigestType, buf); err != nil {
		return err
	}

	return fieldToWire(RDF_C_BINARY, encodeStringToHex(ds.Digest), buf)
}

func DSFromWire(buf *buffer.InputBuffer, ll uint16) (*DS, error) {
