...
func (naptr *NAPTR) Rend(r *MsgRender) error {
	if err := rendField(RDF_C_UINT16, naptr.Order, r); err != nil {
		return err
	}

	if err := rendField(RDF_C_UINT16, naptr.Preference, r); err != nil {
		return err
	}

	if err := rendField(RDF_C_BYTE_BINARY, []byte(naptr.Flags), r); err != nil {
		return err
	}

	if err := rendField(RDF_C_BYTE_BINARY, []byte(naptr.Services), r); err != nil {
		return err
	}

	if err := rendField(RDF_C_BYTE_BINARY, []byte(naptr.Regexp), r); err != nil {
		return err
	}

	return rendField(RDF_C_NAME, naptr.Replacement, r)
}

func (naptr *NAPTR) ToWire(buf *buffer.OutputBuffer) error {
	if err := fieldToWire(RDF_C_UINT16, naptr.Order, buf); err != nil {
		return err
	}

	if err := fieldToWire(RDF_C_UINT16, naptr.Preference, buf); err != nil {
		return err
	}

	if err := fieldToWire(RDF_C_BYTE_BINARY, []byte(naptr.Flags), buf); err != nil {
		return err
	}

	if err := fieldToWire(RDF_C_BYTE_BINARY, []byte(naptr.Services), buf); err != nil {
		return err
	}

	if err := fieldToWire(RDF_C_BYTE_BINARY, []byte(naptr.Regexp), buf); err != nil {
		return err
	}

	return fieldToWire(RDF_C_NAME, naptr.Replacement, buf)
}
...
func NAPTRFromWire(buf *buffer.InputBuffer, ll uint16) (*NAPTR, error) {
