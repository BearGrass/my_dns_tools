...
func (nsec3 *NSEC3) Rend(r *MsgRender) error {
	if r.LeftSpace() < 1+1+2+1 {
		return fmt.Errorf("render has no enough space for nsec3")
	}
	...
	if err := rendField(RDF_C_BINARY, encodeStringToHex(nsec3.Salt), r); err != nil {
		return err
	}

	if err := rendField(RDF_C_UINT8, nsec3.HashLength, r); err != nil {
		return err
	}

	if err := rendField(RDF_C_BINARY, encodeNSEC3NextHash([]byte(nsec3.NextHash)), r); err != nil {
		return err
	}

	if types := encodeNSEC3Bytes(nsec3.Types); len(types) > 0 {
		if err := rendField(RDF_C_BINARY, types, r); err != nil {
			return err
		}
	}
...
func (nsec3 *NSEC3) ToWire(buf *buffer.OutputBuffer) error {
	if buf.LeftSpace() < 1+1+2+1 {
		return fmt.Errorf("render has no enough space for nsec3")
	}
	...
	if err := fieldToWire(RDF_C_BINARY, encodeStringToHex(nsec3.Salt), buf); err != nil {
		return err
	}

	if err := fieldToWire(RDF_C_UINT8, nsec3.HashLength, buf); err != nil {
		return err
	}

	if err := fieldToWire(RDF_C_BINARY, encodeNSEC3NextHash([]byte(nsec3.NextHash)), buf); err != nil {
		return err
	}

	if types := encodeNSEC3Bytes(nsec3.Types); len(types) > 0 {
		if err := fieldToWire(RDF_C_BINARY, types, buf); err != nil {
			return err
		}
	}
...
func NSEC3FromWire(buf *buffer.InputBuffer, ll uint16) (*NSEC3, error) {
