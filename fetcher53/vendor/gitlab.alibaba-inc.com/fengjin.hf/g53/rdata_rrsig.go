...
func (rrsig *RRSig) Rend(r *MsgRender) error {
	if r.LeftSpace() < 2+1+1+4+4+4+2 {
		return fmt.Errorf("render has no enough space for rrsig")
	}
	...
	if err := rendField(RDF_C_NAME, rrsig.Signer, r); err != nil {
		return err
	}

	return rendField(RDF_C_BINARY, rrsig.Signature, r)
}

func (rrsig *RRSig) ToWire(buf *buffer.OutputBuffer) error {
	if buf.LeftSpace() < 2+1+1+4+4+4+2 {
		return fmt.Errorf("render has no enough space for rrsig")
	}
	...
	if err := fieldToWire(RDF_C_NAME, rrsig.Signer, buf); err != nil {
		return err
	}

	return fieldToWire(RDF_C_BINARY, rrsig.Signature, buf)
}
...
func RRSigFromWire(buf *buffer.InputBuffer, ll uint16) (*RRSig, error) {
