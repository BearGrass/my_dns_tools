...
func (soa *SOA) Rend(r *MsgRender) error {
	if err := rendField(RDF_C_NAME, soa.MName, r); err != nil {
		return err
	}

	if err := rendField(RDF_C_NAME, soa.RName, r); err != nil {
		return err
	}

	if r.LeftSpace() < 4+4+4+4+4 {
		return fmt.Errorf("render has no enough space for soa")
	}
...
func (soa *SOA) ToWire(buf *buffer.OutputBuffer) error {
	if err := fieldToWire(RDF_C_NAME, soa.MName, buf); err != nil {
		return err
	}

	if err := fieldToWire(RDF_C_NAME, soa.RName, buf); err != nil {
		return err
	}

	if buf.LeftSpace() < 4+4+4+4+4 {
		return fmt.Errorf("render has no enough space for soa")
	}
...
func SOAFromWire(buf *buffer.InputBuffer, ll uint16) (*SOA, error) {
