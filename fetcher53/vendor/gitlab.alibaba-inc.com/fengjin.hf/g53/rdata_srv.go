...
func (srv *SRV) Rend(r *MsgRender) error {
	if r.LeftSpace() < 2+2+2 {
		return fmt.Errorf("render has no enough space for srv")
	}
	...
	return rendField(RDF_C_NAME_UNCOMPRESS, srv.Target, r)
}

func (srv *SRV) ToWire(buf *buffer.OutputBuffer) error {
	if buf.LeftSpace() < 2+2+2 {
		return fmt.Errorf("render has no enough space for srv")
	}
	...
	return fieldToWire(RDF_C_NAME, srv.Target, buf)
}
...
func SRVFromWire(buf *buffer.InputBuffer, ll uint16) (*SRV, error) {
