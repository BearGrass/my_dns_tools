...
func fieldFromWire(ct RDFCodingType, buf *buffer.InputBuffer, ll uint16) (interface{}, uint16, error) {
	switch ct {
...
func rendField(ct RDFCodingType, data interface{}, render *MsgRender) error {
	switch ct {
	case RDF_C_NAME:
		n, _ := data.(*Name)
		return n.Rend(render)

	case RDF_C_NAME_UNCOMPRESS:
		n, _ := data.(*Name)
		return render.WriteName(n, false)

	case RDF_C_UINT8:
		d, _ := data.(uint8)
		return render.WriteUint8(d)

	case RDF_C_UINT16:
		d, _ := data.(uint16)
		return render.WriteUint16(d)

	case RDF_C_UINT32:
		d, _ := data.(uint32)
		return render.WriteUint32(d)

	case RDF_C_IPV4, RDF_C_IPV6:
		d, _ := data.(net.IP)
		return render.WriteData([]uint8(d))

	case RDF_C_BINARY:
		d, _ := data.([]uint8)
		return render.WriteData(d)

	case RDF_C_TXT:
		ds, _ := data.([]string)
		for _, d := range ds {
			if err := rendField(RDF_C_BYTE_BINARY, []uint8(d), render); err != nil {
				return err
			}
		}
		return nil
	...
	case RDF_C_BYTE_BINARY:
		d, _ := data.([]uint8)
		if err := render.WriteUint8(uint8(len(d))); err != nil {
			return err
		}
		return render.WriteData(d)
	default:
		panic("unknown rdfcoding type")
	}
}

func fieldToWire(ct RDFCodingType, data interface{}, buf *buffer.OutputBuffer) error {
	switch ct {
	case RDF_C_NAME, RDF_C_NAME_UNCOMPRESS:
		n, _ := data.(*Name)
		return n.ToWire(buf)
	...
	case RDF_C_UINT8:
		d, _ := data.(uint8)
		return buf.WriteUint8(d)

	case RDF_C_UINT16:
		d, _ := data.(uint16)
		return buf.WriteUint16(d)

	case RDF_C_UINT32:
		d, _ := data.(uint32)
		return buf.WriteUint32(d)

	case RDF_C_IPV4, RDF_C_IPV6:
		ip, _ := data.(net.IP)
		return buf.WriteData([]uint8(ip))

	case RDF_C_BINARY:
		d, _ := data.([]uint8)
		return buf.WriteData(d)

	case RDF_C_TXT:
		ds, _ := data.([]string)
		for _, d := range ds {
			if err := fieldToWire(RDF_C_BYTE_BINARY, d, buf); err != nil {
				return err
			}
		}
		return nil
	...
	case RDF_C_BYTE_BINARY:
		d, _ := data.([]uint8)
		if err := buf.WriteUint8(uint8(len(d))); err != nil {
			return err
		}
		return buf.WriteData(d)
	default:
