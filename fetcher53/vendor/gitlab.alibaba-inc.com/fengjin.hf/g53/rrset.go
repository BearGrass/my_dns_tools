...
func TTLFromWire(buf *buffer.InputBuffer) (RRTTL, error) {
...
func (ttl RRTTL) Rend(render *MsgRender) error {
	return render.WriteUint32(uint32(ttl))
}

func (ttl RRTTL) ToWire(buf *buffer.OutputBuffer) error {
	return buf.WriteUint32(uint32(ttl))
}
...
func ClassFromWire(buf *buffer.InputBuffer) (RRClass, error) {
...
func (cls RRClass) Rend(render *MsgRender) error {
	return render.WriteUint16(uint16(cls))
}

func (cls RRClass) ToWire(buf *buffer.OutputBuffer) error {
	return buf.WriteUint16(uint16(cls))
}
...
func TypeFromWire(buf *buffer.InputBuffer) (RRType, error) {
...
func (t RRType) Rend(render *MsgRender) error {
	return render.WriteUint16(uint16(t))
}

func (t RRType) ToWire(buf *buffer.OutputBuffer) error {
	return buf.WriteUint16(uint16(t))
}
...
func RRsetFromWire(buf *buffer.InputBuffer) (*RRset, error) {
	var rrset RRset
...
func (rrset *RRset) FromWire(buf *buffer.InputBuffer) error {
...
func (rrset *RRset) Rend(r *MsgRender) error {
	if len(rrset.Rdatas) > 0 {
		for _, rdata := range rrset.Rdatas {
			if err := rrset.Name.Rend(r); err != nil {
				return err
			}
			if r.LeftSpace() < 2+2+4+2 {
				return fmt.Errorf("render doesn't has enough space for rrset")
			}
			rrset.Type.Rend(r)
			rrset.Class.Rend(r)
			rrset.Ttl.Rend(r)
			rdataLenPos := r.Len()
			r.Skip(2)
			if err := rdata.Rend(r); err != nil {
				return err
			}
			r.WriteUint16At(rdataLenPos, uint16(r.Len()-rdataLenPos-2))
		}
	} else {
		if err := rrset.Name.Rend(r); err != nil {
			return err
...
//trim doesn't need during serialization
func (rrset *RRset) ToWire(buf *buffer.OutputBuffer) error {
	if len(rrset.Rdatas) == 0 {
		if err := rrset.Name.ToWire(buf); err != nil {
			return err
		}

		if buf.LeftSpace() < 2+2+4+2 {
			return fmt.Errorf("render doesn't has enough space for rrset")
		}
		...
		for _, rdata := range rrset.Rdatas {
			...
			pos := buf.Len()
			buf.Skip(2)
			if err := rdata.ToWire(buf); err != nil {
				return err
			}
			buf.WriteUint16At(pos, uint16(buf.Len()-pos-2))
		}
	}
